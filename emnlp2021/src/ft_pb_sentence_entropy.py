...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="The learning rate for finetuning the language model."
    )
    parser.add_argument(
        "num_epochs", default=30, type=int,
        help="The number of finetuning epochs."
    )
    parser.add_argument(
    ...
    train_size = 0.7
    train_dataframe = dataframe.sample(frac=train_size, random_state=args.seed)
    ...
    train_dataloader = DataLoader(
        MaptaskSentenceDataset(train_dataframe, tokenizer, args.max_seq_len, args.add_speaker_ids),
    ...
    fine_tuned_model_path = os.path.join(args.out_path, 'pb_fine_tuned_gpt2')
    lm.save_pretrained(fine_tuned_model_path)
    tokenizer.save_pretrained(fine_tuned_model_path)

    print(f'Fine-tuned model saved at {fine_tuned_model_path}')

    lm = GPT2LMHeadModel.from_pretrained(fine_tuned_model_path, return_dict=True)
    lm.to('cuda' if torch.cuda.is_available() else 'cpu')

    out_file_name = compute_entropy(args, test_dataframe)
